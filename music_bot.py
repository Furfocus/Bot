...
async def play_next(guild_id, voice_client):
    ...
    if not voice_client or not voice_client.is_connected():
        print(f"[ERROR] Voice client not connected!")
        player.is_playing = False
        if player.text_channel:
    ...
    try:
        ...
        # Don't recursively call play_next on error, just stop
        player.is_playing = False
...
async def fetch_youtube_info(url):
    ...
    if 'youtu.be' in url:
        ...
        if '/' in url:
            video_id = url.split('/')[-1]  # Get part after last /
            video_id = video_id.split('?')[0]  # Remove everything after ?
            video_id = video_id.split('&')[0]  # Remove everything after &
            url = f"https://youtu.be/{video_id}"
            print(f"[DEBUG] Cleaned short URL: {url}")
    elif 'youtube.com/watch' in url:
        # Handle full URLs: youtube.com/watch?v=VIDEO_ID&other=params
        match = re.search(r'[?&]v=([a-zA-Z0-9_-]{11})', url)
        if match:
            video_id = match.group(1)
            url = f"https://www.youtube.com/watch?v={video_id}"
            print(f"[DEBUG] Cleaned full URL: {url}")
...
async def play(interaction: discord.Interaction, url: str = None, youtuber: str = None):
    ...
    try:
        if not player.voice_client or not player.voice_client.is_connected():
            ...
            print(f"[DEBUG] Connected to voice successfully")
        else:
            print(f"[DEBUG] Already connected to voice")
    except Exception as e:
    ...
    if not player.is_playing:
        ...
        print(f"[DEBUG] Already playing, just added to queue")
